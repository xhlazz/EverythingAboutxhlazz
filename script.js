...
// Typewriter effect that handles HTML tags and rains emoji at the correct moment
function witayTypewriterEffect(text, container) {
  container.innerHTML = ""; // clear
  container.style.display = 'block';
  let i = 0;
  ...
  function type() {
    if (i < text.length) {
      let char = text[i];
      if (char === "<") tag = true;
      if (tag) htmlBuffer += char;
      if (tag && char === ">") {
        tag = false;
        container.innerHTML += htmlBuffer;
        // If the tag is for an emoji, rain it now
        const match = htmlBuffer.match(/<span[^>]*>([^<\w\s])<\/span>/);
        if (match && isEmoji(match[1])) rainEmoji(match[1]);
        htmlBuffer = "";
      } else if (!tag) {
        container.innerHTML += char;
        if (isEmoji(char)) rainEmoji(char);
      }
      i++;
      setTimeout(type, 33);
    } else {
...
// Usage in your submitWitayCode function:
function submitWitayCode() {
  const input = document.getElementById('witay-code-input');
  const code = input.value.trim().toUpperCase();
  const errorDiv = document.getElementById('witay-error');
